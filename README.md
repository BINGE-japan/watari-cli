...
- **Linux / WSLでは bubblewrap（`bwrap`）**。AIのファイル操作とシェルを作業フォルダ内へ
  隔離するために使います。Ubuntuでは `sudo apt install bubblewrap`、Homebrewなら
  `brew install bubblewrap` です。隔離を開始できない場合、ワタリは安全のため道具を実行しません。
...
- AIが使うファイル操作とシェルは、Linux / WSLではOSレベルで隔離されます。見えるのは起動した
  作業フォルダだけで、ホームの認証情報、Windows側、SSH Agent、外部ネットワークには到達できません。
  記憶や接続サービスは、用途を固定した専用処理だけがAIの外側で扱います。予定・メール・チャット・
  タスク等を確認する秘書機能は、この専用処理を通じて従来どおり利用できます。Linearのタスク・
  プロジェクト・コメント等の変更も、任意のAPI命令ではなく用途別の固定処理に変換し、実行内容を
  画面で本人が承認した1操作だけを行います。
- `.env`、秘密鍵、資格情報らしい作業フォルダ内ファイルもAIからは読めません。
- ログイン済みブラウザは、設定で明示したサイトだけを専用処理で扱います。Cookie・デバッグ接続先・
  任意JavaScriptは渡しません。クリックと秘密でない文字入力は、対象と内容を画面に示して本人が承認した
  1操作だけ実行します。パスワード・トークン等の入力は常に拒否し、入力だけでは送信しません。
- `watari chat` はホーム全体ではなく、作業対象のプロジェクトフォルダ内で起動してください。
...
- **Linear** は従来のタスク・プロジェクト管理能力を維持するため、Read / Write / Create issues /
  Create commentsを付け、Adminは付けません。AIにはキーを渡さず、変更は毎回本人確認を挟みます。
