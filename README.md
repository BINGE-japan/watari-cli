...
- ワタリが確認・更新・保存した安全なファイルは、会話の下の **Files** 欄からクリックして
  ファイル管理画面で開けます。秘密らしい名前、隠しファイル、リンク、他人所有のファイルは載せません。
  WSLで使う場合は、`watari chat` の起動時にWindowsの現在ユーザーへ専用リンクの起動設定を登録します。
  リンクは本人専用の鍵で署名され、改ざんされたパスは開きません。
