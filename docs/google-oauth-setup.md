...
- 既定の権限は `drive.appdata`（アプリ専用領域）のみです。**あなたの Drive の他のファイルには
  ...
  これだけです）。
- 同期領域に置かれるのは、あなたとワタリの元の発話テキストです。ツールの出力・思考過程は
  含めませんが、本文中の秘密は自動で除去されません。パスワードなどを会話に入力しないでください。
- 全パソコンで読み終えた分と90日を超えた分は、次回の記憶整理で削除対象になります。
  オフラインや同時更新、更新番号が取得できない場合は削除を見送ります。90日以内の完全削除を保証する
  仕組みではありません。Piの元の会話やGit履歴、AI提供者の保持分は、この削除の対象外です。
