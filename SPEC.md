...
- **セットアップは CLI が全部やる**：ユーザーは `watari install` / `watari <cmd>` を打つだけ。ランタイム(Pi)導入・
  認証・カセット用意まで CLI が orchestrate し、判断が要る所だけ menu（`prompts.select` 等）で出す。
  **生コマンド・フラグ・API キーをユーザーに手組みさせない**（handoff の肝＝素人が1コマンドで動く）。
  各コマンドは不足（Pi 未導入・未認証・カセット無し）を検出したら、その場で補ってから進む自己完結型。
