...
2. 内容を SCHEMA.md の行仕様どおりの JSON 配列にして一時ファイルに書く（ts は現在時刻の UTC、source は "watari"。決めごと・事実は kind:"fact"。profile を付ける場合は、毎回答に必要なら mode:"always"、話題に応じて確認すればよいなら mode:"relevant" を必ず付ける。進行中の事柄は kind:"thread"、学習内容は kind:"study" で domain/topic/mastery/note を必ず入れる）。
